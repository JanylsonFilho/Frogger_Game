...
public class Util {
// o jogador precisa ser testado dentro da area ,por exemplo , Xi do tronco<= x do jogador <= Xf do tronco && Yi do tronco <= y do jogador<= Yf do tronco ,se essa condiçao for verdadeira ,retorna true
    public static boolean colide_com_tronco(Elemento jogador, Elemento tronco){

        if(((tronco.getPx() <= jogador.getPx()) && (jogador.getPx() < tronco.getPx() + (tronco.getLargura())-15) &&
        ((tronco.getPy() <= jogador.getPy()) && (jogador.getPy() < tronco.getPy() + tronco.getAltura()))))
            return true;
